...
CompilerIf #PB_Compiler_Version < 610
	If Not InitScintilla()
		CompilerIf #PB_Compiler_OS = #PB_OS_Windows
			MessageRequester("", "Не найден файл Scintilla.dll")
		CompilerEndIf
		End
	EndIf
CompilerEndIf
...
;-┌───GUI───┐
If OpenWindow(#Window, 0, 0, w, h, "Тест регулярных выражений", #PB_Window_SystemMenu | #PB_Window_ScreenCentered | #PB_Window_SizeGadget | #PB_Window_MinimizeGadget | #PB_Window_MaximizeGadget)
	CompilerIf #PB_Compiler_OS = #PB_OS_Linux
	...
	If tmp < 270
		tmp = 270
	EndIf
	TextGadget(#t1, 10, 1, tmp, 22, "Регулярное выражения для поиска")
	TextGadget(#t2, 10, 19 + SciHeight, 250, 22, "Текст замены")
	TextGadget(#t3, 10, 70 + SciHeight, tmp, 22, "Текст для обработки и результат")
...
;- ├ Buttons
	...
	GadgetToolTip(#btnOpen, "Открыть файл для обработки")
	ButtonImageGadget(#btnClear, w - 270, 155, 30, 30, ImageID(1))
	GadgetToolTip(#btnClear, "Очистить все поля")
	ButtonImageGadget(#btnAddToLib, w - 270, 190, 30, 30, ImageID(3))
	GadgetToolTip(#btnAddToLib, "Добавить/заменить в библиотеку")
	ButtonImageGadget(#btnDel, w - 270, 225, 30, 30, ImageID(4))
	GadgetToolTip(#btnDel, "Удалить пункт из библиотеки")
	ButtonImageGadget(#btnMeta, w - 270, 260, 30, 30, ImageID(5))
	GadgetToolTip(#btnMeta, "Меню метасимволов")
	ButtonImageGadget(#btnUp, w - 270, 295, 30, 30, ImageID(6))
	GadgetToolTip(#btnUp, "Переместить текст вверх")
	ButtonImageGadget(#btnRange, w - 270, 330, 30, 30, ImageID(7))
	GadgetToolTip(#btnRange, "Показать диапазон")

	HyperLinkGadget(#HL_help, w - 80, h - 110, 80, 30, "Справка", RGB(0, 155, 255), #PB_HyperLink_Underline)
	GadgetToolTip(#HL_help, "F1")

	ButtonGadget(#btnStart, w - 270, h - 110, 110, 45, "Старт")
...
;- ├ Menu
	If CreatePopupMenu(#MetaMenu)
		...
		If CreatePopupMenu(#HotMenu)
			mn1 + 1
			MenuItem(mn1, "Старт" + #TAB$ + "Ctr + Enter")
			mn1 + 1
			MenuItem(mn1, "Справка" + #TAB$ + "F1")
...
;- ├ CheckBox
	...
	CompilerSelect #PB_Compiler_OS
		CompilerCase #PB_OS_Windows
			CheckBoxGadget(#Ch_OnTop, tmp, 2, 120, 20, "Поверх")
		CompilerCase #PB_OS_Linux
			CheckBoxGadget(#Ch_OnTop, tmp, 0, 120, 20, "Поверх")
	CompilerEndSelect
	If topmost
		StickyWindow(#Window, #True)
		SetGadgetState(#Ch_OnTop, #PB_Checkbox_Checked)
	EndIf
	CheckBoxGadget(#Ch_NotUpd, tmp, 70 + SciHeight, 120, 20, "не обновлять")
	GadgetToolTip(#Ch_NotUpd, "Не обновлять текст обработки при вставке шаблонов")



	CheckBoxGadget(#Ch_s, w - 270, 10, 250, 20, "(?s) точка всё")
	CheckBoxGadget(#Ch_x, w - 270, 30, 250, 20, "(?x) игнор пробелов и коммент.")
	CheckBoxGadget(#Ch_m, w - 270, 50, 250, 20, "(?m) многостроч. текст (^...$)")
	CheckBoxGadget(#Ch_CRLF, w - 270, 70, 250, 20, "Любой из CR, LF, и CRLF")
	CheckBoxGadget(#Ch_i, w - 270, 90, 250, 20, "(?i) не учитывать регистр")
	SetGadgetState(#Ch_s, 1)
	SetGadgetState(#Ch_CRLF, 1)
	SetGadgetState(#Ch_i, 1)
	GadgetToolTip(#Ch_s, "Точка включает в себя ещё и LF")
	GadgetToolTip(#Ch_CRLF, "Разделение строк может быть любым из этих символов")



	OptionGadget(#Opt_Search, w - 190, h - 240, 190, 20, "Поиск")
	OptionGadget(#Opt_Replace, w - 190, h - 220, 190, 20, "Замена")
	OptionGadget(#Opt_Array, w - 190, h - 200, 190, 20, "Массив")
	OptionGadget(#Opt_Group, w - 190, h - 180, 190, 20, "Группы")
	OptionGadget(#Opt_Step, w - 190, h - 160, 190, 20, "Пошаговый")
	SetGadgetState(#Opt_Array, 1)
	GadgetToolTip(#Opt_Search, "До первого совпадения")
	GadgetToolTip(#Opt_Replace, "Замена всех вхождений")
	GadgetToolTip(#Opt_Array, "Все полные вхождения")
	GadgetToolTip(#Opt_Group, "То что в скобках")
	GadgetToolTip(#Opt_Step, "Ещё позиция и длина")
	...
	GadgetToolTip(#Ch_reg2, "Поддержка групп \1...\9")

	CheckBoxGadget(#Ch_markup, w - 210, h - 140, 220, 20, "С разметкой")
	GadgetToolTip(#Ch_markup, "Помечает начало, полезно для многострочных")
				...
				Select EventGadget()
					...
					Case #HL_help
						CompilerSelect #PB_Compiler_OS
							...
							CompilerCase #PB_OS_MacOS
								RunProgram("open", "http://forum.ru-board.com/topic.cgi?forum=33&topic=0472&start=0&limit=1&m=2#1", "")
						CompilerEndSelect
...
Procedure StartRE()
	...
	If Not Asc(Text\s) Or Not Asc(SciText$)
		SetGadgetText(#Ed_Destination, "Входные данные пусты")
		ProcedureReturn
	EndIf
...
; 	Debug File$
	...
	If hFile
		...
		Else
			MessageRequester("", "Не найден файл шаблона")
		EndIf
	EndIf
EndProcedure
...
Procedure AddToLib()
	...
	EndIf
	tmp$ = InputRequester("Имя", "Укажите имя пункта", tmp$)
	If Asc(tmp$)
		; flag =
		...
		If flgFind And MessageRequester("Перезаписать?", "Пункт " + #q$ + tmp$ + #q$ + " уже существует, перезаписать его?", #PB_MessageRequester_YesNo) = #PB_MessageRequester_Yes
...
Procedure DeleteElemLib()
	...
	If idx <> -1
		sResult$ = GetGadgetItemText(#LV, idx)
		If MessageRequester("Удалить?", "Удалить: " + #q$ + sResult$ + #q$ + "?", #PB_MessageRequester_YesNo) = #PB_MessageRequester_No
			ProcedureReturn
		EndIf
...
Procedure SetExeRegExpToLV(Text$)
	...
	If rex_id
		...
		If Asc(Text$) = '[' ; защита от \D, когда будет выбрано 50000
			...
			Next
		EndIf
		SetGadgetText(#StatusBar, "Найдено: " + Str(Count))
		FreeRegularExpression(rex_id)
	Else
...
; 	EndIf
	...
	If OpenWindow(#WinRange, pos, 0, 320, height, "Тест диапазона", #PB_Window_SystemMenu | #PB_Window_SizeGadget | #PB_Window_MaximizeGadget, WindowID(#Window))
